...
import { validateV3746RuntimeConfig } from "./config/runtimeConfig.js";
import {
...
loadLocalEnv();
validateV3746RuntimeConfig(process.env);
...
const DATA_DIR = process.env.DATA_DIR || path.resolve(process.cwd(), "server", "data-main-v3746");
const MAIN_SETTLEMENT_DIR = path.resolve(process.env.MAIN_SETTLEMENT_DIR || path.join(process.cwd(), "server", "data-main-v3746", "settlements"));
const OBSERVER_CHECKPOINT_DIR = path.resolve(process.env.OBSERVER_CHECKPOINT_DIR || path.join(process.cwd(), "server", "data-observer-v3746", "research", "canonical-checkpoints"));
const OBSERVER_POLICY_PATH = path.resolve(process.env.OBSERVER_POLICY_PATH || path.join(process.cwd(), "server", "data-observer-v3746", "policy", "observer-policy-v3746.json"));
const OBSERVER_STATE_PATH = path.resolve(process.env.OBSERVER_STATE_PATH || path.join(process.cwd(), "server", "data-observer-v3746", "observer", "settlement-cursor-v3746.json"));
const ANTI_DOWNGRADE_REPORT_PATH = path.resolve(process.env.ANTI_DOWNGRADE_REPORT_PATH || path.join(process.cwd(), "server", "data-observer-v3746", "reports", "anti-downgrade-evidence-latest.json"));
...
const V356_RELEASE_VERSION = "v374.6-anti-downgrade-evidence-full-paper";
...
function buildV3745CriticalConfigSnapshot() {
  return {
    ...
    eligibilityProbabilityBasis: "regime_core_independent_asset_probability_with_optional_forward_observer_cap",
...
function applyStrategySelectionToPrediction(prediction, routerDecision, accountRisk) {
  ...
  const observerProbability = observerPolicyReader.effectiveProbability({
...
server.listen(PORT, HOST, async () => {
  ...
  console.log(`[V374.6 ANTI-DOWNGRADE EVIDENCE PAPER CONFIG] ${V3745_CONFIG_HASH} ${JSON.stringify(V3745_CRITICAL_CONFIG)}`);
  ...
  const settlementObserverTimer = settlementObserverLearner
    ? setInterval(runSettlementObserver, SETTLEMENT_OBSERVER_INTERVAL_MS)
    : null;
  scanTimer.unref?.();
  hotLaneTimer?.unref?.();
  uiTickTimer.unref?.();
  settlementObserverTimer?.unref?.();
});
