...
function validateV3746RuntimeConfig(env = process.env) {
  ...
  if (!dataDir || !mainSettlementDir || !observerCheckpointDir || !observerPolicyPath || !observerStatePath) {
    throw new Error("v3745_runtime_data_paths_required");
  }
  ...
  if (env.ANTI_DOWNGRADE_SHADOW_EXECUTION_ENABLED === "1" || env.ANTI_DOWNGRADE_AUTOMATIC_PROMOTION === "1") {
    throw new Error("v3746_unproven_shadow_policy_cannot_execute_or_auto_promote");
  }
  ...
  if (botRole === "main_test") {
    ...
    for (const observerPath of [observerCheckpointDir, observerPolicyPath, observerStatePath, antiDowngradeReportPath]) {
...
const validateV3745RuntimeConfig = validateV3746RuntimeConfig;
const validateV3744RuntimeConfig = validateV3746RuntimeConfig;
