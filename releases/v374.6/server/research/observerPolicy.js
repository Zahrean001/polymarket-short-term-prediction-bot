...
const OBSERVER_POLICY_SCHEMA = "v3746.observer-policy.1";
...
function summarizeCheckpointChallenger(checkpointRows = [], settlements = []) {
  ...
  return {
    ...
    promotionEligible: samples >= 100 && winRate >= 70 && unitRoiPct > 0,
    automaticMainExecutionPromotion: false,
  };
}

function createSettlementObserverLearner(options = {}) {
  const settlementDirectory = path.resolve(options.settlementDirectory || "server/data-main-v3746/settlements");
  const checkpointDirectory = path.resolve(options.checkpointDirectory || "server/data-observer-v3746/research/canonical-checkpoints");
  const policyPath = path.resolve(options.policyPath || "server/data-observer-v3746/policy/observer-policy-v3746.json");
  const statePath = path.resolve(options.statePath || "server/data-observer-v3746/observer/settlement-cursor-v3746.json");
  ...
  function runOnce(now = Date.now()) {
    ...
    atomicWriteJson(statePath, {
      schemaVersion: "v3746.settlement-cursor.1",
...
function validateObserverPolicy(policy, options = {}, now = Date.now()) {
  ...
  if (policy.reliability?.reliable !== true || policy.reliability?.promotionEligible !== true) {
    return { valid: false, reason: "observer_policy_not_reliable", ageMs };
  }
  return { valid: true, reason: "observer_policy_reliable_and_fresh", ageMs };
}
