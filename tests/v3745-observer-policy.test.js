...
test("fresh 100-sample positive observer evidence remains telemetry-only and cannot cap main probability", () => {
  const dir = fixture();
  ...
  assert.equal(result.policy.reliability.evidenceGatePassed, true);
  assert.equal(result.policy.reliability.promotionEligible, false);
  assert.equal(validateObserverPolicy(result.policy).valid, false);
  assert.equal(validateObserverPolicy(result.policy).reason, "observer_policy_telemetry_only_execution_forbidden");
  ...
  assert.equal(probability.applied, false);
  assert.equal(probability.effectiveProbability, 0.90);
  assert.match(probability.reason, /telemetry_only_execution_forbidden/);
});
