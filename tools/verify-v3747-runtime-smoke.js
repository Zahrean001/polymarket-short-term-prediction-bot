...
const app = ecosystem.apps.find((item) => item.name === "polymarket-main-v3747");
if (!app) throw new Error("v3747_main_process_missing");
...
const runtimeRoot = fs.mkdtempSync(path.join(os.tmpdir(), "v3747-fastgrow-smoke-"));
...
const child = spawn(process.execPath, ["server/index.js"], {
  ...
  env: {
    ...
    MAIN_SIGNAL_DIR: path.join(dataDir, "signals"),
    ...
    OBSERVER_POLICY_PATH: path.join(observerDataDir, "policy", "observer-policy-v3747.json"),
    OBSERVER_STATE_PATH: path.join(observerDataDir, "observer", "settlement-cursor-v3747.json"),
    ANTI_DOWNGRADE_REPORT_PATH: path.join(observerDataDir, "reports", "anti-downgrade-evidence-latest.json"),
    DIRECTION_SHADOW_DECISION_DIR: path.join(observerDataDir, "research", "direction-only-decisions"),
    DIRECTION_SHADOW_STATE_PATH: path.join(observerDataDir, "observer", "direction-only-cursor-v3747.json"),
    DIRECTION_SHADOW_REPORT_PATH: path.join(observerDataDir, "reports", "direction-only-shadow-latest.json"),
...
try {
  ...
  for (let attempt = 0; attempt < 80; attempt += 1) {
    ...
    if (child.exitCode !== null) throw new Error(`v3747_exited_early:${child.exitCode}\n${output}`);
    try {
      const response = await fetch(`http://127.0.0.1:${port}/health`);
      if (response.ok) { health = await response.json(); break; }
    } catch {}
  }
  if (!health?.ok) throw new Error(`v3747_health_not_ready\n${output}`);
  if (health.botRole !== "main_test") throw new Error(`v3747_role_wrong:${health.botRole}`);
  if (health.botInstanceId !== "main-direction-shadow-fastgrow-v3747") throw new Error("v3747_instance_wrong");
  if (health.releaseVersion !== "v374.7-direction-only-shadow-fastgrow-full-paper") throw new Error(`v3747_release_wrong:${health.releaseVersion}`);
  if (!/^[a-f0-9]{64}$/.test(health.configHash || "")) throw new Error("v3747_config_hash_missing");
  if (health.paperState?.persistenceEnabled !== true) throw new Error("v3747_main_state_persistence_inactive");
  if (health.directionShadowObserver?.status !== "disabled_for_main") throw new Error("v3747_direction_writer_active_in_main");
  if (path.resolve(health.paperState?.path || "") !== path.resolve(dataDir, "btc-paper-state.json")) {
    throw new Error("v3747_main_state_path_not_isolated");
  }
  ...
  if (!recoveredFeed) {
    throw new Error(`v3747_silent_external_feed_not_recovered:connections=${externalFeedConnections}\n${output}`);
  }

  const configResponse = await fetch(`http://127.0.0.1:${port}/api/v3/effective-config`);
  const config = await configResponse.json();
  const critical = config.criticalConfig || {};
  if (critical.mainBehaviorPolicyId !== "v3745-frozen-control") throw new Error("v3747_main_behavior_not_frozen");
  if (critical.antiDowngradeShadowExecutionEnabled !== false || critical.antiDowngradeAutomaticPromotion !== false) {
    throw new Error("v3747_unproven_shadow_policy_can_affect_main");
  }
  if (
    critical.directionShadowEnabled !== false ||
    critical.directionShadowExecutionEnabled !== false ||
    critical.directionShadowAutomaticPromotion !== false
  ) throw new Error("v3747_direction_shadow_can_affect_main");
  if (critical.eligibilityProbabilityBasis !== "regime_core_independent_asset_probability_frozen_no_observer_execution") {
    throw new Error("v3747_main_probability_basis_not_frozen");
  }
  ...
  console.log(`V374.7 direction-only shadow fast-grow main runtime smoke PASSED on port ${port}`);
} finally {
  await stop();
