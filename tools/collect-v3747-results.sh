...
MAIN_DATA_DIR="${MAIN_DATA_DIR:-$ROOT_DIR/server/data-main-v3747}"
OBSERVER_DATA_DIR="${OBSERVER_DATA_DIR:-$ROOT_DIR/server/data-observer-v3747}"
MAIN_PORT="${CHALLENGER_BOT_PORT:-8799}"
OUTPUT_DIR="${1:-$HOME}"
STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
NAME="v3747-complete-results-$STAMP"
...
cp -a "$MAIN_DATA_DIR" "$STAGE_DIR/data-main-v3747"
if [[ -d "$OBSERVER_DATA_DIR" ]]; then
  cp -a "$OBSERVER_DATA_DIR" "$STAGE_DIR/data-observer-v3747"
fi

pm2 logs polymarket-main-v3747 --lines 10000 --nostream > "$STAGE_DIR/main-pm2.log" 2>&1 || true
pm2 logs polymarket-direction-observer-v3747 --lines 10000 --nostream > "$STAGE_DIR/observer-pm2.log" 2>&1 || true
pm2 describe polymarket-main-v3747 > "$STAGE_DIR/main-pm2-describe.txt" 2>&1 || true
pm2 describe polymarket-direction-observer-v3747 > "$STAGE_DIR/observer-pm2-describe.txt" 2>&1 || true
